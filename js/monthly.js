...
export function initMonthlyMode() {
  ...
  switchMonthlyTab('tab-stock'); 
  updateOnlineUI(); 
  toggleLoader(true);
  fetchBackend('getMonthlyInitData').then(res => {
    monthlyDrugMaster = res.drugMaster; 
    monthlyTables = res.tables;
    document.getElementById('monthly-table-select').innerHTML = '<option value="">請選擇盤點表...</option>' + monthlyTables.map(t => `<option value="${t.id}">${t.name}</option>`).join('');
    loadUserRecords(() => { toggleLoader(false); });
  }).catch(err => { toggleLoader(false); alert("載入主檔失敗"); });
}
...
export function generateRecordCards(recordsArray, emptyMsg, allowEdit) { if (recordsArray.length === 0) return `<div class="text-center p-3 text-muted fw-bold">${emptyMsg}</div>`; let html = ''; recordsArray.forEach(record => { let qtyStr = record.handQty; let colorClass = 'text-primary'; let dispBadge = ''; if (record.dispType === '調劑') { qtyStr = `-${record.handQty}`; colorClass = 'text-danger'; dispBadge = `<span class="badge bg-danger ms-1">調劑</span>`; } else if (record.dispType === '退藥') { qtyStr = `+${record.handQty}`; colorClass = 'text-success'; dispBadge = `<span class="badge bg-success ms-1">退藥</span>`; } const locInfo = record.loc ? ` | 儲位: ${record.loc}` : ''; const editButtons = allowEdit ? `<div class="d-flex justify-content-between align-items-center mt-1 pt-1 border-top"><div class="fs-5 fw-bold ${colorClass}">${qtyStr}</div><div><button class="btn btn-sm btn-outline-primary py-0 me-1" onclick="editRecord('${record.sn}', '${record.handQty}', '${record.dispType}', '${record.loc}')">修改</button><button class="btn btn-sm btn-outline-danger py-0" onclick="deleteRecord('${record.sn}', '${record.loc}')">作廢</button></div></div>` : `<div class="fs-5 fw-bold ${colorClass} mt-1 pt-1 border-top">${qtyStr}</div>`; html += `<div class="card mb-2 shadow-sm border-0 border-start border-4 border-info"><div class="card-body p-2"><div class="d-flex justify-content-between mb-1"><div class="fw-bold text-dark text-truncate" style="max-width: 70%;">${record.name}</div><div class="small text-muted" style="font-size:0.75rem;">${record.time}</div></div><div class="small text-secondary" style="font-size:0.8rem;"><span class="badge bg-secondary">${record.type}</span>${dispBadge}<span class="ms-1">代碼: ${record.code}${locInfo}</span></div>${editButtons}</div></div>`; }); return html; }
...
export function renderMonthlyDashboard() {
      ...
      <div class="card mb-3 shadow-sm border-0 border-start border-4 ${isComplete ? 'border-success' : 'border-warning'}">
        <div class="card-body p-3">
          ...
          <div class="d-flex justify-content-between align-items-center">
            <div class="small text-secondary">已盤: ${counted} | 未盤: ${total - counted}</div>
            <button class="btn btn-sm btn-outline-academic fw-bold" onclick="showTableDetailModal('${table.id}', '${table.name}')">明細</button>
          </div>
        </div>
      </div>`;
  ...
  unfinishedArea.innerHTML = unfinishedHtml || '<div class="text-center text-muted py-3">暫無進行中的盤點表</div>';
  finishedArea.innerHTML = finishedHtml || '<div class="text-center text-muted py-3">尚無完成的盤點表</div>';
}
...
export function showTableDetailModal(tableId, tableName) {
  ...
  const tbodyHtml = table.items.map(item => {
    // 優先抓取 GAS 帶下來的資料，若無才去 userRecord 撈，雙重防呆解決 undefined
